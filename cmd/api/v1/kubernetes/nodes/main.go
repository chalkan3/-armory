...
import (
	...
	"scheduler/internal/api/v1/kubernetes/nodes"
	nodelistner "scheduler/internal/schedule/v1/listners/node"
...
var eventListeners = scheduler.Listeners{
	"create-node-master-primary": nodelistner.MasterPrimary,
	"create-node-master":         nodelistner.Master,
	"create-node-worker":         nodelistner.Worker,
	"destroy-node":               nodelistner.DeleteNode,
}

func main() {
	fs := flag.NewFlagSet("", flag.ExitOnError)
	var (
		httpPort    = fs.String("http_port", "9011", "application http port default 9000")
	...
	repository := nodes.NewRepository(database)

	routes := nodes.NewHTTPServer(nodes.NewService(repository, scheduler), logger)
