...
func (nc *CreateCommand) Create(cmd *cobra.Command, args []string) {
	...
	manifest := v1beta.Postgres{
		ApiVersion: "postgres/v1beta",
		Spec: &v1beta.Spec{
			ClusterName: cluster,
	...
	payload, err := json.Marshal(map[string]v1beta.Postgres{
		"manifest": manifest,
	})
	...
	resp, err := http.Post("http://localhost:9003/v1/database/postgres", "application/json",
